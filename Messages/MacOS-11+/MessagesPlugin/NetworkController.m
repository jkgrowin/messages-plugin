...
- (void)connect {
    ...
    int port = 28080;
    DLog("MessagesPlugin: Connecting to socket on port %{public}d", port);
    // connect to socket
    asyncSocket = [[GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:dispatch_get_main_queue()];
    NSError *err = nil;
    if (![asyncSocket connectToHost:@"localhost" onPort:port error:&err]) {
        // If there was an error, it's likely something like "already connected" or "no delegate set"
        DLog("MessagesPlugin: Error connecting to socket: %{public}@", err);
    }
...
- (void)sendMessage:(NSDictionary*)data {
    ...
    DLog("MessagesPlugin: Sending data: %{public}@", data);
    [asyncSocket writeData:(finalData) withTimeout:(-1) tag:(1)];
}

- (void)socket:(GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port {
    DLog("MessagesPlugin: socket:%{public}p didConnectToHost:%{public}@ port:%{public}hu", sock, host, port);
...
- (void)socketDidSecure:(GCDAsyncSocket *)sock {
    DLog("MessagesPlugin: socketDidSecure:%{public}p", sock);
}

// called when a message is sent to the server
- (void)socket:(GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag {
    DLog("MessagesPlugin: socket:%p didWriteDataWithTag:%ld", sock, tag);
}

- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag {
    NSString* str = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    DLog("MessagesPlugin: event: %{public}@ socket:%{public}p didReadData:withTag:%{public}ld", str, sock, tag);
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err {
    DLog("MessagesPlugin: Disconnected from server, attempting to reconnect in 5 seconds... (%{public}@)", err);
    double delayInSeconds = 5.0;
    dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        DLog("MessagesPlugin: Attempting to reconnect");
        [self connect];
    });
}
